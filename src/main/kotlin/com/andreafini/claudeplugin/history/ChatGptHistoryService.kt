...
/**
 * Cronologia delle interazioni con ChatGPT, persistente per progetto
 * (file `.idea/chatGptAssistantHistory.xml`). Parallela e indipendente da
 * [ClaudeHistoryService].
 */
@State(name = "ChatGptHistory", storages = [Storage("chatGptAssistantHistory.xml")])
class ChatGptHistoryService : PersistentStateComponent<ChatGptHistoryService.State> {
    ...
    companion object {
        ...
        fun getInstance(project: Project): ChatGptHistoryService =
            project.getService(ChatGptHistoryService::class.java)
    }
}
