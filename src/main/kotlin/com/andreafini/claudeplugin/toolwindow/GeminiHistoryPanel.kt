...
/** Pannello della tool window Gemini: conversazioni + interazioni + dettaglio + azioni. */
class GeminiHistoryPanel(private val project: Project) : JBPanel<GeminiHistoryPanel>(BorderLayout()) {

    private val history = GeminiHistoryService.getInstance(project)
    private val listModel = DefaultListModel<GeminiHistoryService.Interaction>()
    private val list = object : JBList<GeminiHistoryService.Interaction>(listModel) {
    ...
    init {
        ...
        top.add(JButton("Svuota cronologia").apply {
            addActionListener {
                val answer = Messages.showYesNoDialog(
                    project, "Svuotare tutta la cronologia?", "Gemini Assistant", null,
                )
        ...
        list.cellRenderer = object : ColoredListCellRenderer<GeminiHistoryService.Interaction>() {
            override fun customizeCellRenderer(
                list: JList<out GeminiHistoryService.Interaction>,
                value: GeminiHistoryService.Interaction,
                index: Int,
                selected: Boolean,
                hasFocus: Boolean,
            ) {
     ...
     */
    private fun tooltipHtml(interaction: GeminiHistoryService.Interaction): String {
    ...
    private fun reload() {
        ...
        val sorted = items.sortedWith(
            compareByDescending<GeminiHistoryService.Interaction> { lastTs[it.conversationId] ?: 0L }
    ...
    private fun showDetail(interaction: GeminiHistoryService.Interaction?) {
        ...
        if (interaction == null) {
            ...
            val info = GeminiPricing.infoLine(
            ...
            buttons.add(JButton("Inserisci nel file").apply {
                addActionListener {
                    ...
                    val ok = GeminiActionSupport.insertIntoEditor(project, null, interaction.response)
                    if (!ok) {
                        GeminiActionSupport.notifyError(project, "Nessun file aperto in cui inserire.")
                    }
                }
            })
