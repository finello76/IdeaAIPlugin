...
import com.andreafini.claudeplugin.action.ChatGptActionSupport
...
/** Pannello della tool window ChatGPT: conversazioni + interazioni + dettaglio + azioni. */
class ChatGptHistoryPanel(private val project: Project) : JBPanel<ChatGptHistoryPanel>(BorderLayout()) {

    private val history = ChatGptHistoryService.getInstance(project)
    private val listModel = DefaultListModel<ChatGptHistoryService.Interaction>()
    private val list = object : JBList<ChatGptHistoryService.Interaction>(listModel) {
    ...
    init {
        ...
        top.add(JButton("Svuota cronologia").apply {
            addActionListener {
                val answer = Messages.showYesNoDialog(
                    project, "Svuotare tutta la cronologia?", "ChatGPT Assistant", null,
                )
        ...
        list.cellRenderer = object : ColoredListCellRenderer<ChatGptHistoryService.Interaction>() {
            override fun customizeCellRenderer(
                list: JList<out ChatGptHistoryService.Interaction>,
                value: ChatGptHistoryService.Interaction,
                index: Int,
                selected: Boolean,
                hasFocus: Boolean,
            ) {
     ...
     */
    private fun tooltipHtml(interaction: ChatGptHistoryService.Interaction): String {
    ...
    private fun reload() {
        ...
        val sorted = items.sortedWith(
            compareByDescending<ChatGptHistoryService.Interaction> { lastTs[it.conversationId] ?: 0L }
    ...
    private fun showDetail(interaction: ChatGptHistoryService.Interaction?) {
        ...
        if (interaction == null) {
            ...
            val info = ChatGptPricing.infoLine(
            ...
            buttons.add(JButton("Inserisci nel file").apply {
                addActionListener {
                    ...
                    val ok = ChatGptActionSupport.insertIntoEditor(project, null, interaction.response)
                    if (!ok) {
                        ChatGptActionSupport.notifyError(project, "Nessun file aperto in cui inserire.")
                    }
                }
            })
    ...
    private fun rerunButton(interaction: ChatGptHistoryService.Interaction): JButton {
        val button = JButton("Rilancia su ▾")
        button.toolTipText = "Invia la stessa richiesta a un altro motore AI configurato"
        button.addActionListener {
            val targets = CrossEngine.availableTargets(CrossEngine.Engine.CHATGPT)
            if (targets.isEmpty()) {
            ...
            if (promptToSend.isBlank()) {
                ChatGptActionSupport.notifyError(project, "Questa interazione non ha un prompt da rilanciare.")
                return@addActionListener
            }
            val menu = JPopupMenu()
